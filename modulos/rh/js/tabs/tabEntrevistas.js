...
import { arrayUnion, serverTimestamp } from "../../../../assets/js/firebase-init.js";
...
const btnRegistrarEntrevista = document.getElementById("btn-registrar-entrevista-rh");
...
window.abrirModalEntrevistaRH = function (candidatoId, dadosCandidato) {
    ...
    modalEntrevistaRH.dataset.candidaturaId = candidatoId;
    ...
    document.getElementById("entrevista-rh-nome-candidato").textContent = nomeCompleto;
    
    // 🔴 CORREÇÃO 3: Injetar Status Atual e Resumo da Triagem
    document.getElementById("entrevista-rh-status-atual").textContent = statusAtual;
    document.getElementById("entrevista-rh-resumo-triagem").textContent = resumoTriagem;

    const btnVerCurriculo = document.getElementById("entrevista-rh-ver-curriculo");
    if (btnVerCurriculo) {
...
async function submeterAvaliacaoEntrevistaRH(e) {
    ...
    const { candidatosCollection, currentUserData, handleTabClick, statusCandidaturaTabs } = state;
    ...
    const resultado = form.querySelector('input[name="resultado_entrevista"]:checked')?.value;
    ...
    btnRegistrarEntrevista.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i> Processando...';

    // 2. Determinar Status e Próxima Etapa
    const isAprovado = resultado === "Aprovado";
    const novoStatusCandidato = isAprovado ? "Entrevista RH Aprovada (Testes Pendente)" : "Rejeitado (Comunicação Pendente)";
    const abaRecarregar = state.statusCandidaturaTabs.querySelector('.tab-link.active').getAttribute('data-status');
    ...
    try {
        ...
        await updateDoc(candidaturaRef, {
            ...
            historico: arrayUnion({
                ...
                acao: `Entrevista RH: ${isAprovado ? 'APROVADO' : 'REPROVADO'}. Status: ${novoStatusCandidato}`,
                usuario: currentUserData.id || "rh_system_user",
            }),
        });
        
        window.showToast(`Avaliação de Entrevista RH registrada. Status: ${novoStatusCandidato}`, "success");

        // 3. Envio da Mensagem de WhatsApp (com agendamento)
        if (dataEntrevista && horaEntrevista && dadosCandidatoEntrevista.telefone_contato) {
            const mensagem = encodeURIComponent(
                `Olá ${dadosCandidatoEntrevista.nome_completo || 'candidato(a)'}! Sua entrevista com RH foi agendada para o dia ${dataEntrevista} às ${horaEntrevista}. Por favor, confirme sua presença.`
            );
            const telefoneLimpo = dadosCandidatoEntrevista.telefone_contato.replace(/\D/g, '');
        ...
        const activeTab = statusCandidaturaTabs.querySelector(`[data-status="${abaRecarregar}"]`);
        ...
        btnRegistrarEntrevista.innerHTML = '<i class="fas fa-check-circle me-2"></i> Registrar e Enviar para Próxima Fase';
    }
}
...
if (modalEntrevistaRH) {
    ...
    if (btnRegistrarEntrevista) {
        btnRegistrarEntrevista.removeEventListener("click", submeterAvaliacaoEntrevistaRH);
        btnRegistrarEntrevista.addEventListener("click", submeterAvaliacaoEntrevistaRH);
    }
    
    // 2. Fechamento do Modal (X no cabeçalho e botão 'Voltar ao Painel')
    document.querySelectorAll("[data-modal-id='modal-entrevista-rh']").forEach(btn => {
        btn.addEventListener("click", () => modalEntrevistaRH.classList.remove("is-visible"));
    });
    
    // Listener para o botão 'Voltar ao Painel' no footer
    const btnVoltarPainel = modalEntrevistaRH.querySelector('#btn-voltar-painel-rh');
    if (btnVoltarPainel) {
        btnVoltarPainel.addEventListener("click", () => modalEntrevistaRH.classList.remove("is-visible"));
    }
}