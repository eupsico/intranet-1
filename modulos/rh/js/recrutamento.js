...
window.abrirModalAvaliacaoTriagem = function (candidatoId, dadosCandidato) {
  ...
  // 🟢 NOVO CAMPO: Popula o campo de informação para Aprovação
...
async function submeterAvaliacaoTriagem(e) {
  ...
  const infoAprovacaoEl = document.getElementById("modal-info-aprovacao"); // ID do novo campo
  ...
  const dadosAvaliacao = {
    ...
    // 🟢 NOVO CAMPO SALVO: Informação Adicional na Aprovação
...
* CORRIGIDO: O listener do botão fechar é anexado aqui, após o botão ser inserido.
...
async function abrirModalCandidato(candidatoId, modo, candidato) {
 ...
 if (btnFechar) {
   btnFechar.addEventListener("click", () => {
     ...
     // Opcional: Recarregar a aba ativa, mas para o modal de detalhes não é essencial
...
export async function initRecrutamento(user, userData) {
 ...
 // O listener foi movido para dentro de abrirModalCandidato para lidar com o DOM dinâmico.
 ...
 if (modalAvaliacaoTriagem) {
   ...
   // 🟢 Lógica de mostrar/esconder os campos Motivo da Rejeição E o novo campo Info da Aprovação
   ...
   // 🟢 NOVO CONTAINER PARA APROVAÇÃO (ID do HTML Alterado)
   const aprovacaoContainer = document.getElementById("modal-info-aprovacao-container");
   
   // Torna a função global para que abrirModalAvaliacaoTriagem possa usá-la.
   window.toggleMotivoAprovacaoRejeicao = () => {
     if (radioNao.checked) {
       motivoContainer.style.display = "block";
       motivoTextarea.required = true;
       if (aprovacaoContainer) aprovacaoContainer.style.display = "none";
     } else if (radioSim.checked) { // EXIBIR A CAIXA DE TEXTO PARA INFORMAÇÕES DE APROVAÇÃO
       motivoContainer.style.display = "none";
       motivoTextarea.required = false;
       if (aprovacaoContainer) aprovacaoContainer.style.display = "block";
     } else {
       motivoContainer.style.display = "none";
       motivoTextarea.required = false;
       if (aprovacaoContainer) aprovacaoContainer.style.display = "none";
   ...
   // 🟢 Usar a nova função no listener
