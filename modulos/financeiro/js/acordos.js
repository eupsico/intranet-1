...
export function init(user, userData) {
  ...
  const getDividaTotal = (profissional, dbData) => {
    ...
    const mesFinal =
      hoje.getDate() > 10 ? hoje.getMonth() : hoje.getMonth() - 1;
  ...
  const generateContract = (content, acordoSalvo = {}) => {
    const parcelas =
      parseInt(content.querySelector(".acordo-parcelas").value) || 0;
    const entrada =
      parseFloat(content.querySelector(".acordo-entrada").value) || 0;
    const valorFinal =
      parseFloat(content.querySelector(".acordo-valor-final").value) || 0;
    const parcelasTable = content.querySelector(".parcelas-table");
    const dataAcordoInput = content.querySelector(".acordo-data").value;
    const dataEntradaInput = content.querySelector(
      ".acordo-data-entrada"
    ).value;
    ...
    if (entrada > 0) {
      const pagamentoSalvo = acordoSalvo.pagamentos?.find(
        (p) => p.item === "Entrada"
      );
      const dataVencimento = dataEntradaInput;
      const dataPagamento = pagamentoSalvo ? pagamentoSalvo.dataPagamento : "";
      const nomeMes = dataVencimento
        ? meses[new Date(dataVencimento + "T00:00:00").getMonth()]
        : "";
      tbody.innerHTML += `<tr>
            <td>Entrada</td>
            <td>${
            ...
            <td><button class="action-button whatsapp-parcela-btn" data-item="Entrada" data-valor="${entrada.toFixed(
              2
            )}" data-vencimento="${dataVencimento}">Enviar</button></td>
        </tr>`;
    }
    if (parcelas > 0) {
      let calculationBaseDate =
        entrada > 0 && dataEntradaInput
          ? new Date(dataEntradaInput + "T00:00:00")
          : new Date(dataAcordoInput + "T00:00:00");
      const valorParcela = valorFinal / parcelas;
      for (let i = 0; i < parcelas; i++) {
        const mesVencimento = new Date(
          calculationBaseDate.getFullYear(),
          calculationBaseDate.getMonth() + 1 + i,
          10
        );
        const nomeMes =
          meses[mesVencimento.getMonth()].charAt(0).toUpperCase() +
          meses[mesVencimento.getMonth()].slice(1);
        const itemParcela = `Parcela ${i + 1}/${parcelas}`;
        const pagamentoSalvo = acordoSalvo.pagamentos?.find(
          (p) => p.item === itemParcela
        );
        const dataVencimento = pagamentoSalvo
          ? pagamentoSalvo.dataVencimento || pagamentoSalvo.data
          : mesVencimento.toISOString().split("T")[0];
        const dataPagamento = pagamentoSalvo
          ? pagamentoSalvo.dataPagamento
          : "";
        tbody.innerHTML += `<tr>
                <td>${itemParcela}</td>
                <td>${nomeMes}</td>
                ...
                <td><input type="date" class="data-vencimento" value="${dataVencimento}" readonly></td>
                <td><input type="date" class="data-pagamento-efetivo" value="${dataPagamento}"></td>
                <td><button class="action-button whatsapp-parcela-btn" data-item="${itemParcela}" data-valor="${valorParcela.toFixed(
          2
        )}" data-vencimento="${dataVencimento}">Enviar</button></td>
            </tr>`;
      }
  ...
  const calculateAndUpdate = (content) => {
    ...
    const divida =
      parseFloat(content.querySelector(".acordo-divida").value) || 0;
    const entrada =
      parseFloat(content.querySelector(".acordo-entrada").value) || 0;
  ...
  const renderAccordions = (devedores) => {
    ...
    if (devedores.length === 0) {
      appContent.innerHTML =
        "<p>Nenhum profissional com dívidas pendentes.</p>";
      return;
    }
    ...
    devedores.forEach((devedor) => {
      ...
      accordion.innerHTML = `
            <button class="accordion-trigger">${
              devedor.nome
            }  (Dívida: R$ ${devedor.valor.toFixed(2)})</button>
            <div class="accordion-content">
                <div class="form-grid">
                    <div class="form-group"><label>Valor Dívida (R$)</label><input class="acordo-divida" value="${devedor.valor.toFixed(
                      2
                    )}" readonly></div>
                    <div class="form-group"><label>Data do Acordo</label><input type="date" class="acordo-data" value="${
                      acordoSalvo.dataAcordo || hoje
                    }"></div>
                    <div class="form-group"><label>Entrada (R$)</label><input type="number" step="0.01" class="acordo-entrada" value="${
                      acordoSalvo.entrada || ""
                    }"></div>
                    <div class="form-group"><label>Pagamento Entrada</label><input type="date" class="acordo-data-entrada" value="${
                      acordoSalvo.dataEntrada || hoje
                    }"></div>
                    <div class="form-group"><label>Valor Final (R$)</label><input class="acordo-valor-final" value="${
                      acordoSalvo.valorFinal || devedor.valor.toFixed(2)
                    }" readonly></div>
                    <div class="form-group"><label>Nº Parcelas</label><input type="number" class="acordo-parcelas" value="${
                      acordoSalvo.parcelas || ""
                    }"></div>
                </div>
  ...
  async function fetchData() {
    ...
    try {
      ...
      const listaDevedores = DB.profissionais
        ...
        .map((prof) => ({
          nome: prof.nome,
          valor: getDividaTotal(prof, DB).valor,
        }))
  ...
  function attachEventListeners() {
    taxaGeralInput.addEventListener("input", async () => {
      const configRef = doc(db, "financeiro", "configuracoes");
      await setDoc(
        configRef,
        { valores: { taxaAcordo: parseFloat(taxaGeralInput.value) || 0 } },
        { merge: true }
      );
      appContent
        .querySelectorAll(".accordion-content.active")
        .forEach(calculateAndUpdate);
    });
    ...
    appContent.addEventListener("click", async (e) => {
      ...
      if (target.classList.contains("delete-btn")) {
        ...
        if (
          confirm(
            `Tem certeza que deseja excluir permanentemente o acordo de ${nome}? Esta ação não pode ser desfeita.`
          )
        ) {
          try {
            await deleteDoc(doc(db, "acordos", nomeKey));
            window.showToast(
              `Acordo de ${nome} excluído com sucesso!`,
              "success"
            );
            fetchData();
          } catch (err) {
      ...
      if (target.classList.contains("save-btn")) {
        ...
        const pagamentos = [];
        accordionContext
          .querySelectorAll(".parcelas-table tbody tr")
          .forEach((row) => {
            pagamentos.push({
        ...
        const acordoData = {
          ...
          dataEntrada: accordionContext.querySelector(".acordo-data-entrada")
            .value,
          divida:
            parseFloat(
              accordionContext.querySelector(".acordo-divida").value
            ) || 0,
          entrada:
            parseFloat(
              accordionContext.querySelector(".acordo-entrada").value
            ) || 0,
          taxa: parseFloat(taxaGeralInput.value) || 0,
          valorFinal:
            parseFloat(
              accordionContext.querySelector(".acordo-valor-final").value
            ) || 0,
          parcelas:
            parseInt(
              accordionContext.querySelector(".acordo-parcelas").value
            ) || 0,
          pagamentos: pagamentos,
          timestamp: serverTimestamp(),
        };
        try {
          await setDoc(doc(db, "acordos", nomeKey), acordoData);
          window.showToast(`Acordo para ${nome} salvo com sucesso!`, "success");
          const index = DB.acordos.findIndex((a) => a.prof === nome);
          if (index > -1) {
            DB.acordos[index] = acordoData;
          } else {
            DB.acordos.push(acordoData);
          }
          fetchData();
        } catch (err) {
      ...
      if (target.classList.contains("whatsapp-btn")) {
        ...
        if (!contato) {
          return alert(`Contato para ${nome} não encontrado.`);
        }
        ...
        content.querySelectorAll(".parcelas-table tbody tr").forEach((row) => {
          const [item, , valor, dataVenc] = Array.from(row.cells).map(
        ...
        const template =
          DB.Mensagens.contrato ||
          "{{greeting}}, {{nomeProfissional}}!\n\nSegue um resumo do seu acordo com a EuPsico.\n\n- Valor da Dívida: R$ {{divida}}\n- Entrada: R$ {{entrada}}\n- Taxa Aplicada: {{taxa}}%\n- Valor Final: R$ {{valorFinal}}\n- Nº de Parcelas: {{parcelas}}\n\nAs parcelas serão:\n{{listaParcelas}}";
        const mensagemFinal = template
          .replace("{{greeting}}", getGreeting())
          .replace(/{{nomeProfissional}}/g, nome)
          .replace("{{divida}}", divida.toFixed(2).replace(".", ","))
          .replace("{{entrada}}", entrada.toFixed(2).replace(".", ","))
          .replace("{{taxa}}", taxa)
          .replace("{{valorFinal}}", valorFinal.toFixed(2).replace(".", ","))
          .replace("{{parcelas}}", parcelas)
          .replace("{{listaParcelas}}", listaParcelasTexto.trim());

        window.open(
          `https://wa.me/55${contato}?text=${encodeURIComponent(
            mensagemFinal
          )}`,
          "_blank"
        );
      ...
      if (target.classList.contains("whatsapp-parcela-btn")) {
        ...
        const mensagemParcela = template
          .replace("{{greeting}}", getGreeting())
          .replace(/{{nome}}/g, nome)
          .replace("{{item}}", item)
          .replace("{{vencimento}}", `${dia}/${mes}/${ano}`)
          .replace("{{valor}}", valor.toFixed(2).replace(".", ","))
          .replace(
            "{{valorRestante}}",
