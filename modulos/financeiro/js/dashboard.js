// Arquivo: /modulos/financeiro/js/dashboard.js
...
export function init() {
  ...
  function updateDashboard() {
    ...
    const lancamentosPagosNoMes = allLancamentos.filter(
      (l) =>
        l.dataPagamento &&
        new Date(l.dataPagamento).getUTCMonth() === mes &&
        new Date(l.dataPagamento).getUTCFullYear() === ano
    );
    const contasAtrasadas = allLancamentos.filter(
      (l) =>
        l.tipo === "despesa" &&
        l.status === "pendente" &&
        l.dataVencimento < hoje
    );
    const receitasPagas = lancamentosPagosNoMes
      .filter((l) => l.tipo === "receita")
      .reduce((sum, l) => sum + l.valor, 0);
    const despesasPagas = lancamentosPagosNoMes
      .filter((l) => l.tipo === "despesa")
      .reduce((sum, l) => sum + l.valor, 0);
    const despesasAtrasadasTotal = contasAtrasadas.reduce(
      (sum, l) => sum + l.valor,
      0
    );
    ...
    document.getElementById("saldo-total").textContent = formatCurrency(
      receitasPagas - despesasPagas
    );
    document.getElementById("total-atrasadas").textContent = formatCurrency(
      despesasAtrasadasTotal
    );
  ...
  function renderMonthlyTables(mes, ano) {
    const despesasTbody = document
      .getElementById("contas-a-pagar-table")
      ?.querySelector("tbody");
    const receitasTbody = document
      .getElementById("entradas-table")
      ?.querySelector("tbody");
    ...
    const despesas = lancamentosDoPeriodo
      .filter((l) => l.tipo === "despesa")
      .sort((a, b) => new Date(a.dataVencimento) - new Date(b.dataVencimento));
    const receitas = lancamentosDoPeriodo
      .filter((l) => l.tipo === "receita")
      .sort((a, b) => new Date(a.dataVencimento) - new Date(b.dataVencimento));
    ...
    despesasTbody.innerHTML =
      despesas.length > 0
        ? despesas
            .map(
              (l) =>
                `<tr><td>${formatDate(l.dataVencimento)}</td><td>${
                  l.status === "pago" ? formatDate(l.dataPagamento) : "Pendente"
                }</td><td>${l.descricao}</td><td>${
                  l.categoria
                }</td><td class="text-despesa">${formatCurrency(
                  l.valor
                )}</td><td>${
                  l.status === "pago"
                    ? "Pago"
                    : `<div class="pay-action-group"><input type="date" class="pay-date-input" value="${
                        new Date().toISOString().split("T")[0]
                      }"><button class="action-button save-payment-btn" data-id="${
                        l.id
                      }">Pagar</button></div>`
                }</td></tr>`
            )
    ...
    receitasTbody.innerHTML =
      receitas.length > 0
        ? receitas
            .map(
              (l) =>
                `<tr><td>${formatDate(l.dataVencimento)}</td><td>${formatDate(
                  l.dataPagamento
                )}</td><td>${l.descricao}</td><td>${
                  l.categoria
                }</td><td class="text-receita">${formatCurrency(
                  l.valor
                )}</td></tr>`
            )
            .join("")
  ...
  function updateAtrasoTable(contasAtrasadas) {
    const tbody = document
      .getElementById("atraso-table")
      ?.querySelector("tbody");
    if (!tbody) return;
    contasAtrasadas.sort(
        ...
        : contasAtrasadas
            .map(
              (l) =>
                `<tr><td>${formatDate(l.dataVencimento)}</td><td>${
                  l.descricao
                }</td><td>${l.categoria}</td><td>${formatCurrency(
                  l.valor
                )}</td></tr>`
            )
            .join("");
  ...
  async function handleSavePayment(e) {
    ...
    if (dataPagamento) {
      try {
        const docRef = doc(db, "fluxoCaixa", lancamentoId);
  ...
  function updateCharts(lancamentosPagosNoMes) {
    ...
    const evolucaoCtx = document
      .getElementById("evolucaoMensalChart")
      ?.getContext("2d");
    if (evolucaoCtx)
      chartInstances.evolucaoMensalChart = createEvolucaoChart(evolucaoCtx);
    const despesasCtx = document
      .getElementById("despesasCategoriaChart")
      ?.getContext("2d");
    if (despesasCtx)
      chartInstances.despesasCategoriaChart = createDespesasCategoriaChart(
        despesasCtx,
        lancamentosPagosNoMes
      );
    const receitasCtx = document
      .getElementById("receitasCategoriaChart")
      ?.getContext("2d");
    if (receitasCtx)
      chartInstances.receitasCategoriaChart = createReceitasCategoriaChart(
        receitasCtx,
        lancamentosPagosNoMes
      );
  }
  ...
  function createEvolucaoChart(ctx) {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const lancamentosPagos = allLancamentos.filter(
        (l) =>
          l.dataPagamento &&
          new Date(l.dataPagamento).getMonth() === mes &&
          new Date(l.dataPagamento).getFullYear() === ano
      );
      receitasData.push(
        lancamentosPagos
          .filter((l) => l.tipo === "receita")
    ...
    return new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          {
            data: Object.values(data),
            backgroundColor: ["#28a745", "#20c997", "#90EE90", "#198754"],
          },
        ],
      },
      options: { responsive: true, maintainAspectRatio: false },
  ...
  function generatePDFReport() {
    ...
    const doc = new jsPDF();
    const mes = document.getElementById("dashboard-mes-selector").options[
      document.getElementById("dashboard-mes-selector").selectedIndex
    ].text;
    ...
    const totalAtrasadas =
      document.getElementById("total-atrasadas").textContent;
    doc.autoTable({
      startY: 30,
      theme: "plain",
      body: [
  ...
  const q = query(fluxoCaixaRef, orderBy("dataVencimento", "desc"));
