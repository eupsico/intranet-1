...
// *** ALTERAÇÃO: Corrigido erro de serverTimestamp() em array na função criarSessoesRecorrentes ***
      ...
      switch (tipo) {
        case "novas_sessoes":
          ...
          setTextContentIfExists(
            ...
            detalhes.recorrenciaSolicitada || "Não informado"
          );
          // *************************************************
          ...
          // ***** NOVO: Pré-preenche a recorrência no select do admin *****
          const recorrenciaSelectAdmin = modalBodyContent.querySelector(
            "#admin-ag-recorrencia"
          );
          if (recorrenciaSelectAdmin && detalhes.recorrenciaSolicitada) {
            const recorrSoliLower =
              detalhes.recorrenciaSolicitada.toLowerCase();
            if (
              recorrenciaSelectAdmin.querySelector(
                `option[value="${recorrSoliLower}"]`
              )
            ) {
              recorrenciaSelectAdmin.value = recorrSoliLower;
            }
          }
          // ************************************************************
          break;
    ...
    try {
      ...
      if (novosDados.alterarGrade === "Sim") {
        ...
        await atualizarGradeDoProfissional(
          ...
          novosDados.dia, // 'Segunda-feira', etc.
