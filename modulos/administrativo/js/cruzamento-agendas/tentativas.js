...
function listenToTentativas() {
  ...
  unsubscribe = onSnapshot(q, (snapshot) => {
    ...
    tentativas.forEach((t) => {
      ...
      // Define os dados para uso nos modais e lógica
      ...
      // Aplica a cor da linha baseada no status imediatamente
      ...
      tr.innerHTML = `
                ...
                <td class="text-center">
                    <button class="btn btn-sm btn-outline-danger btn-excluir-tentativa" title="Excluir esta tentativa">
...
function updateRowColor(tr, status) {
  ...
  // Gera o slug do status (ex: "Primeiro Contato" -> "status-primeiro-contato")
  if (status) {
    const slug = status
      .toLowerCase()
      .trim()
  ...
  newBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (fila === "Plantão") {
        ...
        updateData["plantaoInfo.tipoAtendimento"] = mod;
      } else {
        // Para PB, geralmente já existe o array, mas podemos adicionar se necessário
        // updateData["atendimentosPB"] = ... (Lógica complexa omitida, foca no status)
      }
